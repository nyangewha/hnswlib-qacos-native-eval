...
class Index {
 public:
    ...
    void set_doc_sign_sketches(py::object ids_obj, py::object packed_codes_obj, size_t sketch_bits) {
        py::array_t<hnswlib::labeltype, py::array::c_style | py::array::forcecast> ids(ids_obj);
        py::array_t<uint64_t, py::array::c_style | py::array::forcecast> packed_codes(packed_codes_obj);
        auto ids_buf = ids.request();
        auto codes_buf = packed_codes.request();
        if (ids_buf.ndim != 1) {
            throw std::runtime_error("ids must be a 1D array");
        }
        if (codes_buf.ndim != 2) {
            throw std::runtime_error("packed_codes must be a 2D array of shape [num_docs, code_words]");
        }
        size_t rows = (size_t)ids_buf.shape[0];
        if ((size_t)codes_buf.shape[0] != rows) {
            throw std::runtime_error("ids and packed_codes row counts must match");
        }
        size_t code_words = (size_t)codes_buf.shape[1];
        std::vector<uint64_t> ordered(rows * code_words, 0);
        auto* ids_ptr = (hnswlib::labeltype*)ids_buf.ptr;
        auto* codes_ptr = (uint64_t*)codes_buf.ptr;
        for (size_t i = 0; i < rows; ++i) {
            auto it = appr_alg->label_lookup_.find(ids_ptr[i]);
            if (it == appr_alg->label_lookup_.end()) {
                throw std::runtime_error("label not found in index while attaching doc sketches");
            }
            size_t internal = (size_t)it->second;
            std::memcpy(ordered.data() + internal * code_words, codes_ptr + i * code_words, code_words * sizeof(uint64_t));
        }
        appr_alg->setExperimentalDocSignSketches(ordered.data(), rows, code_words, sketch_bits);
    }

    py::object knn_query_experimental(
        ...
        size_t* upper_hops = new size_t[rows];
        size_t* lower_pops = new size_t[rows];
        ...
        py::capsule free_when_done_d(data_numpy_d, [](void* f) { delete[] (float*)f; });
        py::capsule free_visited(visited_nodes, [](void* f) { delete[] (size_t*)f; });
        py::capsule free_upper(upper_hops, [](void* f) { delete[] (size_t*)f; });
        py::capsule free_pops(lower_pops, [](void* f) { delete[] (size_t*)f; });
        ...
        py::list visited_candidate_dists_py;
        for (size_t i = 0; i < rows; ++i) {
            py::list one;
            for (auto id : coarse_id_lists[i]) one.append(py::int_(id));
            coarse_ids_py.append(one);

            py::list visited_ids_one;
            for (auto id : visited_candidate_id_lists[i]) visited_ids_one.append(py::int_(id));
            visited_candidate_ids_py.append(visited_ids_one);

            py::list visited_dists_one;
            for (auto d : visited_candidate_dist_lists[i]) visited_dists_one.append(py::float_(d));
            visited_candidate_dists_py.append(visited_dists_one);
        }

        py::dict info;
        info["visited_nodes"] = py::array_t<size_t>({rows}, {sizeof(size_t)}, visited_nodes, free_visited);
        info["upper_hops"] = py::array_t<size_t>({rows}, {sizeof(size_t)}, upper_hops, free_upper);
        info["lower_pops"] = py::array_t<size_t>({rows}, {sizeof(size_t)}, lower_pops, free_pops);
...
PYBIND11_PLUGIN(hnswlib) {
        ...
        .def("set_doc_sign_sketches",
            &Index<float>::set_doc_sign_sketches,
            py::arg("ids"),
            py::arg("packed_codes"),
            py::arg("sketch_bits"))
        .def("knn_query_experimental",
            &Index<float>::knn_query_experimental,
            py::arg("query_sign_packed"),
            py::arg("query_proj") = py::none(),
            py::arg("k") = 1,
            py::arg("num_threads") = -1,
