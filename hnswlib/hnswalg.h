...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
    ...
    void setExperimentalDocSignSketches(
        const uint64_t* packed_codes,
        size_t num_elements,
        size_t code_words,
        size_t sketch_bits) {
        if (packed_codes == nullptr) {
            throw std::runtime_error("packed_codes must not be null");
        }
        if (num_elements != cur_element_count) {
            throw std::runtime_error("num_elements must match current index element count");
        }
    ...
    inline dist_t computeExperimentalSearchDistance_(
        ...
        ExperimentalSearchScorer scorer) const {
...
//                    if (candidate_id == 0) continue;
    ...
    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
    searchBaseLayerSTExperimental(
        tableint ep_id,
        const ExperimentalQueryScorerCtx& ctx,
        size_t ef,
        ExperimentalSearchScorer scorer,
        BaseFilterFunctor* isIdAllowed = nullptr,
        ExperimentalSearchStats* stats = nullptr,
        std::vector<labeltype>* coarse_candidate_labels = nullptr,
        std::vector<labeltype>* visited_candidate_labels = nullptr,
        std::vector<dist_t>* visited_candidate_distances = nullptr) const {
        VisitedList *vl = visited_list_pool_->getFreeVisitedList();
        vl_type *visited_array = vl->mass;
        vl_type visited_array_tag = vl->curV;
        ...
        if (bare_bone_search || (!isMarkedDeleted(ep_id) && ((!isIdAllowed) || (*isIdAllowed)(getExternalLabel(ep_id))))) {
            dist_t dist = computeExperimentalSearchDistance_(ep_id, ctx, scorer);
            if (visited_candidate_labels && visited_candidate_distances) {
                visited_candidate_labels->push_back(getExternalLabel(ep_id));
                visited_candidate_distances->push_back(dist);
            }
            lowerBound = dist;
            top_candidates.emplace(dist, ep_id);
            candidate_set.emplace(-dist, ep_id);
            if (stats) { stats->visited_nodes += 1; stats->lower_pushes += 1; }
        } else {
            lowerBound = std::numeric_limits<dist_t>::max();
        ...
        while (!candidate_set.empty()) {
            ...
            for (size_t j = 1; j <= size; j++) {
                ...
                if (!(visited_array[candidate_id] == visited_array_tag)) {
                    visited_array[candidate_id] = visited_array_tag;
                    if (stats) { stats->visited_nodes += 1; }

                    dist_t dist = computeExperimentalSearchDistance_(candidate_id, ctx, scorer);
                    if (visited_candidate_labels && visited_candidate_distances) {
                        visited_candidate_labels->push_back(getExternalLabel(candidate_id));
                        visited_candidate_distances->push_back(dist);
                    }
                    bool flag_consider_candidate = top_candidates.size() < ef || lowerBound > dist;
                    if (flag_consider_candidate) {
                        candidate_set.emplace(-dist, candidate_id);
                        if (stats) { stats->lower_pushes += 1; }
        ...
        if (coarse_candidate_labels) {
            auto copy = top_candidates;
            std::vector<labeltype> ordered_labels;
            ordered_labels.reserve(copy.size());
            while (!copy.empty()) {
                ordered_labels.push_back(getExternalLabel(copy.top().second));
                copy.pop();
            }
            coarse_candidate_labels->assign(ordered_labels.rbegin(), ordered_labels.rend());
        }
        if (stats) {
            stats->coarse_result_size = top_candidates.size();
            if (visited_candidate_labels) {
                stats->visited_candidate_pool_size = visited_candidate_labels->size();
            }
        }
        visited_list_pool_->releaseVisitedList(vl);
        return top_candidates;
...
//                    if (candidate_id == 0) continue;
    ...
    */
    ...
    searchKnnExperimental(
        ...
        tableint currObj = enterpoint_node_;
        dist_t curdist = computeExperimentalSearchDistance_(enterpoint_node_, ctx, scorer);
        ...
        for (int level = maxlevel_; level > 0; level--) {
            ...
            while (changed) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (cand < 0 || cand > max_elements_)
                        throw std::runtime_error("cand error");
                    dist_t d = computeExperimentalSearchDistance_(cand, ctx, scorer);
                    if (d < curdist) {
                        curdist = d;
                        currObj = cand;
                        changed = true;
        ...
        bool bare_bone_search = !num_deleted_ && !isIdAllowed;
        if (bare_bone_search) {
            top_candidates = searchBaseLayerSTExperimental<true>(currObj, ctx, std::max(ef_, k), scorer, isIdAllowed, stats, coarse_candidate_labels, visited_candidate_labels, visited_candidate_distances);
        } else {
            top_candidates = searchBaseLayerSTExperimental<false>(currObj, ctx, std::max(ef_, k), scorer, isIdAllowed, stats, coarse_candidate_labels, visited_candidate_labels, visited_candidate_distances);
        }
